...
<div class="containerr">
    <div class="heading"> Настройки классного руководства </div>
...
<button id="showFormButton" class="login-button" style="margin-right: 5px;">Назначить руководителя</button>
            <button class="login-button" style="margin-right: 5px; display: none;" id="toggleFormButton">Свернуть форму</button>
            <button onclick="toggleTable()" class="login-button" style="margin-right: 5px;">Внести изменения</button>
            <button onclick="toggleTableAdd()" class="login-button" style="margin-right: 5px;">Удалить руководителя</button>

        </div>';

// Query to fetch teachers not present in the manual table
$stmt_available_teachers = $dbh->prepare("
    SELECT t.id, t.fio 
    FROM teachers t 
    LEFT JOIN manual m ON t.id = m.id_teacher 
    WHERE m.id_teacher IS NULL
");
$stmt_available_teachers->execute();
$available_teachers = $stmt_available_teachers->fetchAll(PDO::FETCH_ASSOC);

// Query to fetch classes not present in the manual table
$stmt_available_classes = $dbh->prepare("
    SELECT id, name 
    FROM class 
    WHERE id NOT IN (SELECT id_class FROM manual)
");
$stmt_available_classes->execute();
$available_classes = $stmt_available_classes->fetchAll(PDO::FETCH_ASSOC);

// Display available teachers and classes in the select dropdowns
echo '<form id="dataForm" style="display: none;" method="post" action="vender/class_manual.php">';
echo '<table border="0">';
echo '<tr><th>Выберите учителя</th><th>Выберите класс</th><th>Назначить</th></tr>';
echo '<tr>';
echo '<td><select name="teacher_name" class="iinput">';
...
echo '<td><select name="class_name" class="iinput">';
echo '<option value="" selected disabled></option>';
...
echo "<form action='vender/upDateClass_ruk.php' method='post'>";

$stmt = $dbh->prepare("SELECT m.id as manual_id, t.fio as teacher_fio, t.id as teacher_id, c.id as class_id, c.name as class_name
                      FROM manual m
                      INNER JOIN class c ON m.id_class = c.id
                      INNER JOIN teachers t ON m.id_teacher = t.id");
$stmt->execute();
$students = $stmt->fetchAll(PDO::FETCH_ASSOC);

echo "<input class='input' type='text' id='searchInput' placeholder='Поиск по ФИО, классу'>";

echo "<table id='poisk' border='0'>";
echo "<tr><th>Классный руководитель</th><th>Класс</th><th>Изменить</th></tr>";

foreach ($students as $student) {
    echo '<tr>';
    
    echo '<td>';
    echo '<select class="iinput" name="teacher_id['.$student["manual_id"].']">';
    echo '<option value="">Выберите учителя</option>';
    // Выводим список всех учителей
    $stmt = $dbh->prepare("SELECT id, fio FROM teachers");
    $stmt->execute();
    ...
    foreach ($teachers as $teacher) {
        ...
        echo '>' . $teacher["fio"] . '</option>';
    }
    echo '</select>';
    echo '</td>';

    echo '<td>';
    echo '<select class="iinput" name="class_id['.$student["manual_id"].']">';
    echo '<option value="">Выберите класс</option>';
    // Выводим список всех классов
    $stmt = $dbh->prepare("SELECT id, name FROM class");
    $stmt->execute();
    ...
    echo '<td><button type="submit" class="login-button" name="submit['.$student["manual_id"].']" value="submit">🖊️</button></td>';
...
<div id="studentsTableAdd" style="display:none;">
    <div id="studentsTable" style="display:block;">
        <form id="deleteForm" method="post" action="vender/del_class_ruk.php">
            <?php
            $stmt = $dbh->prepare("SELECT m.id, t.fio as teacher_fio, t.id as teacher_id, c.id as class_id, c.name as class_name
                                  FROM manual m
                                  INNER JOIN class c ON m.id_class = c.id
                                  INNER JOIN teachers t ON m.id_teacher = t.id");
            $stmt->execute();
            ...
            echo "<input class='input' type='text' id='searchInput' placeholder='Поиск по ФИО, классу'>";
            echo "<table id='poisk' border='0'>";
            echo "<tr><th>Классный руководитель</th><th>Класс</th><th>Удалить</th></tr>";

            foreach ($students as $student) {
                echo '<tr>';
                echo '<td>' . $student["teacher_fio"] . '</td>';
