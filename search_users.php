...
<div class="heading">Пользователи</div>

    <form action="search_users.php" method="GET">
            <div class="group">
             <input placeholder="Введите ФИО пользователя" type="search" class="input" name="query">
             ...
             <?php   
                ...
                $id = $_GET['id'];
                  $query = $_GET['query'];
                  $rows = $dbh->query("SELECT users.id as user_id, users.fio, rule.id as rule_id, rule.name, users.login, users.password 
                    FROM users 
                    INNER JOIN rule ON users.rule = rule.id 
        ...
        <div class='card'>
            <div class='card-info'>
                <div class='filmcontainer'>
                    <div class='maininfo'>
                        <div class='ssilka'>
                            <p  class='title'>".$row['fio']." <hr> ".$row['name']."</p>
                        </div>
                        <p class='movie-description'>Логин:  ".$row['login']."<br> Пароль: ".$row['password']."</p>";

    if ($row['rule_id'] != 4) {
        echo "
            <a style='padding:1%; ' href='children.php?id=" . $row['user_id']    . "' class='inputbutton'>Добавить ребенка</a>";
    }

    echo "
            <a style='padding:1%; ' href='update_users.php?id=" . $row['user_id'] . "' class='inputbutton'>Изменить данные</a>
            <a style='padding:1%; ' href='vender/delite_user.php?id=" . $row['user_id'] . "' class='inputbutton'>Удалить пользователя</a>
        </div>
    </div>
</div>
</div>";
