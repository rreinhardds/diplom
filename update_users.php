...
<div class="heading">Изменить данные о пользователе</div>

    <form action="vender/updateUsers.php" method="POST" enctype="multipart/form-data">
        <div class="card">
            <div class="card-info">
        ...
        $id = $_GET['id'];
        $rows = $dbh->query("SELECT * from users where users.id=$id");
        $array = $rows->fetch(PDO::FETCH_ASSOC);
        ?>


     
       <input type="hidden" name="id" value="<?= $id?>" class="inputt">
        <label>ФИО пользователя:</label><br>
        <input type="text" class="input" name="fio" value="<?php echo $array['fio']; ?>"><br>
 
        <label>Логин пользователя</label><br>
        <input type="text" class="input" name="login" value="<?php echo $array['login']; ?>"><br>

        <input type="hidden" name="rule" value="<?php echo $array['rule']; ?>" class="inputt">

        <label>Пароль пользователя:</label><br>
        <input type="text" class="input" name="password"  value="<?php echo $array['password']; ?>"><br>

       <input class="input" type="submit" value="Сохранить изменения✔️">
