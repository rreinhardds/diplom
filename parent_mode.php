...
<div class="heading">
    ...
    // Проверка наличия пользователя в таблице parents
    $stmt_check_parent = $dbh->prepare("
        SELECT s.id AS student_id, s.fio AS student_name
        FROM students s
        JOIN parents p ON s.id = p.id_student
        WHERE p.id_user = :id_user
    ");
    $stmt_check_parent->bindValue(':id_user', $_SESSION['id']);
    $stmt_check_parent->execute();

    if ($stmt_check_parent->rowCount() > 0) {
        // Пользователь найден в таблице parents, выводим информацию
        foreach ($stmt_check_parent as $row) {
            echo "<div class='men'><a href='parens_dnem.php?student_id={$row['student_id']}'>Перейти в журнал<h5>Ребёнок:<hr> " . $row['student_name'] . "</h5></a></div>";
        }
    } else {
