...
<div class="containerr">
    <div class="heading"> Настройки классов </div>
...
<button id="showFormButton" class="login-button" style="margin-right: 5px;">Добавить новый класс</button>
            <button class="login-button" style="margin-right: 5px; display: none;" id="toggleFormButton">Свернуть форму</button>
            <button onclick="toggleTable()" class="login-button" style="margin-right: 5px;">Добавить изменения</button>
            <button onclick="toggleTableAdd()" class="login-button" style="margin-right: 5px;">Добавить ученика в класс</button>

        </div>
        <form id="dataForm" style="display: none;" method="post" action="vender/add_class.php">
            <label for="name required">Наименование класса:</label>
            <input class="input" type="text" autocomplete="off" placeholder="Пример: 8а" pattern="^(?=.*\d{1,11})(?=.*[а-я])[0-9а-я]+$" id="name" name="name"><br><br>
...
$stmt = $dbh->prepare("SELECT s.id, s.fio, c.id as class_id, c.name
                            FROM students s
                            INNER JOIN class c ON s.id_class = c.id");
$stmt->execute();
$students = $stmt->fetchAll();
echo " <input class='input' type='text' id='searchInput' placeholder='Поиск по ФИО'>";

echo "<table id='poisk' border='0'>";
echo "<tr><th>ФИО</th><th>Класс</th><th>Изменить класс</th></tr>";

foreach ($students as $student) {
    echo "<tr>";
    echo "<td>".$student['fio']."</td>";
    echo "<td>
            <select class='iinput' name='class_".$student['id']."'>";
            $stmt = $dbh->prepare("SELECT id, name FROM class");
            $stmt->execute();
            $classes = $stmt->fetchAll();
            foreach ($classes as $class) {
                $selected = ($student['class_id'] == $class['id']) ? 'selected' : '';
                echo "<option value='".$class['id']."' ".$selected.">".$class['name']."</option>";
            }
    echo "</select></td>
            <td><button class='login-button class-button' type='button' onclick='changeClass(".$student['id'].")'>🖊️</button>
          </td>";
    echo "</tr>";
}
...
<div id='studentsTableAdd' style='display:none;'>
    <form method="post" action="vender/add_student_in_class.php">
        <?php
        $stmt = $dbh->prepare("SELECT u.id, u.fio, r.name
        FROM users u
        INNER JOIN rule r ON u.rule = r.id
        WHERE u.rule = 4 AND NOT EXISTS (
            SELECT 1
            FROM students s
            WHERE s.fio = u.fio
        )");
        $stmt->execute();
        $studentsAdd = $stmt->fetchAll();
echo " <input class='input' type='text' id='searchInputt' placeholder='Поиск по ФИО'>";
        echo "<table id= 'add_poisk' border='0'>";
        echo "<tr><th>ФИО</th><th>Роль</th><th>Класс</th><th>Добавить</th></tr>";

        foreach ($studentsAdd as $studentAdd) {
            echo "<tr>";
            echo "<td>" . htmlspecialchars($studentAdd['fio']) . "</td>";
            echo "<td>" . $studentAdd['name'] . "</td>";
            echo "<td>
            <select class='iinput' name='id_class'>";
            $stmt = $dbh->prepare("SELECT id, name FROM class");
            $stmt->execute();
            $classes = $stmt->fetchAll();

            // Sorting classes alphabetically and by numerical order in the format "8a", "6b"
            usort($classes, function($a, $b) {
                $classA = preg_replace('/\D/', '', $a['name']);
                $classB = preg_replace('/\D/', '', $b['name']);
                $strCmp = strcmp($classA, $classB);
                return $strCmp === 0 ? strnatcasecmp($a['name'], $b['name']) : $strCmp;
            });
            ...
            echo "</select>
                <input type='hidden' name='fio' value='" . htmlspecialchars($studentAdd['fio']) . "'>
            </td>";
            echo "<td><button class='login-button' onclick=\"addToClass('" . htmlspecialchars($studentAdd['fio']) . "', this.parentNode.previousElementSibling.firstElementChild.value)\">+</button></td>";
            echo "</tr>";
        }
